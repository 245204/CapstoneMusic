...
public class MusicController {
	...
	public ResponseEntity<List<MusicEntity>> getMusicByYear(@PathVariable Long musicYear) throws NoAlbumFoundException {
		...
		if (music.isEmpty()) {
			//return new ResponseEntity<>(HttpStatus.NOT_FOUND);
			throw new NoAlbumFoundException("There is no music Published at this year");
	...
	public ResponseEntity<MusicEntity> getMusicById(@PathVariable int musicId) throws NoAlbumFoundException {
		MusicEntity music = musicService.getMusicById(musicId);
		if (music != null) {
			return new ResponseEntity<>(music, HttpStatus.OK);
		} else {
			throw new NoAlbumFoundException("Id not present");
			//return new ResponseEntity<>(HttpStatus.NOT_FOUND);
		}
	}
